...
用上方 starter 的初始條件 `y0 = radians([100, -20, 30, 0, 0, 0])`、`DOP853`（`rtol=1e-10, atol=1e-12`,
`max_step=0.01`）跑 10 秒，你應得到（與硬體無關的量級）：

| 量 | 參考值 |
|---|---|
| 初始總能量 $E_0$ | $\approx -21.82$ J |
| 相對能量範圍 `rel_range_pct` | $\approx 3.6\times10^{-9}\,\%$（遠 $< 1\%$ 里程碑）|
| $M(q)$ 對稱、正定 | 全 ROM 皆成立 |

落在 $10^{-8}\%$ 量級即通過；若 $>1\%$，多半是**手推方程的符號錯誤**或積分容差太鬆。
